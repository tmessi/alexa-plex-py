...
deploy: aplexa.zip
	aws lambda update-function-code --zip-file fileb://aplexa.zip --function-name $(LAMBDA_FUNC_NAME)
...
requirements.txt: $(VE_DIR)/bin/activate requirements.in
	$(VE_DIR)/bin/pip-compile requirements.in
	$(VE_DIR)/bin/pip-sync requirements.txt
